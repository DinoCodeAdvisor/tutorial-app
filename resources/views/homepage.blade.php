...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex items-center lg:justify-center min-h-screen flex-col">
        ...
        <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow starting:opacity-0 h-full">
            <main class="flex gap-10 justify-between w-full px-20 py-10 flex-col lg:flex-row">
                <div class="text-[13px] flex flex-col p-6 pb-12 lg:p-20 bg-white dark:bg-[#161615] dark:text-[#EDEDEC] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-bl-lg rounded-br-lg lg:rounded-tl-lg lg:rounded-br-none ">
                    <img src="{{asset('storage/skinmc-user.png')}}" alt="User avatar" class="w-3xs">
                    <h1 class="mb-1 font-bold text-4xl text-center">{{$user->name}}</h1>
                    <h2 class="mb-4 font-medium text-xl text-center">({{$user->email}})</h2>
                    <hr class="mb-4">
                    <div class="w-full flex flex-row gap-4 justify-center items-center">
                        <div class="block max-w-sm p-6 text-center hover:bg-[#41413f] rounded-sm">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{$user->post->count()}}</h5>
                            <p class="font-normal text-xl text-gray-700 dark:text-gray-400">Posts</p>
                        </div>
                        <div class="block max-w-sm p-6 text-center hover:bg-[#41413f] rounded-sm">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{$user->comment->count()}}</h5>
                            <p class="font-normal text-xl text-gray-700 dark:text-gray-400">Comments</p>
                        </div>
                    </div>
                </div>
                
                <div class="text-[13px] leading-[20px] flex-1 w-lg text-start p-6 pb-12 lg:p-20 bg-white dark:bg-[#161615] dark:text-[#EDEDEC] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-bl-lg rounded-br-lg lg:rounded-tl-lg lg:rounded-br-none ">
                    <h1 class="text-3xl font-bold">Available Posts</h1>
                    <hr class="my-4">
                    <ul>
                        @foreach ($posts as $p)
                            <li class="mb-4">
                                <a href="{{route('posts.show', $p)}}" class="block p-6 text-start hover:bg-[#41413f] rounded-sm max-w-md xl:max-w-6xl">
                                    <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white truncate">{{$p->title}}</h5>
                                    <p class="font-normal text-lg text-gray-700 dark:text-gray-400 truncate">{{$p->content}}</p>
                                </a>
                            </li>
                        @endforeach
                    </ul>
                    <div class="w-full flex items-center justify-center">
                        <a href="{{route('posts.create')}}" class="flex gap-1 px-5 py-3 text-center text-white bg-[#f52f037c] focus:ring-4 focus:outline-none font-medium rounded-lg text-sm w-full sm:w-auto dark:bg-[#f6150056] dark:hover:bg-[#f52f03b0] dark:focus:ring-[#F61500]">
